...
switch ($current_go_case) {
        ...
        if (!ip('submit')) {
            ...
            if (empty(p('rname')) && $NOT_USER) {
                $ERRORS['rname'] =
                    $lang['EMPTY_FIELDS'] .
                    ' : ' .
                    (empty(p('rname')) && $NOT_USER ? ' [ ' . $lang['YOURNAME'] . ' ] ' : '') .
        ...
        if (ip('submit')) {
            ...
            if ((empty(p('cname')) && $NOT_USER) || empty(p('ctext'))) {
                $ERRORS['cname'] =
                    $lang['EMPTY_FIELDS'] .
                    ' : ' .
                    (empty(p('cname')) && $NOT_USER ? ' [ ' . $lang['YOURNAME'] . ' ] ' : '') .
            ...
            if (ig('sure') && g('sure') == 'ok') {
                ...
                $extra_codes =
                    '<script type="text/javascript">
                        ...
                        {
                            if(confirm(\'' .
                    $lang['ARE_YOU_SURE_DO_THIS'] .
                    '\')){
                                window.location = "go.php?go=del&sure=ok&cd=' .
                    $cd .
                    '";
                            }else{
        ...
        switch (g('case')):
            ...
            case 'sync_files':
                ...
                if (!$end) {
                    ...
                    $text = $title =
                        sprintf($lang['SYNCING'], $lang['ALL_FILES']) .
                        ' (' .
                        (!$start ? 0 : $start) .
                        '->' .
                        (!$end ? '?' : $end) .
            ...
            case 'sync_images':
                ...
                if (!$end) {
                    delete_cache('data_stats');
                    $text = $title =
                        sprintf($lang['SYNCING_DONE'], $lang['ALL_IMAGES']) .
                        ' (' .
                        (!$start ? 0 : $start) .
                        '->' .
                        (!$end ? '?' : $end) .
    ...
    case 'uploading_progress':
        ...
        $result_data['current_progress'] =
            ($progress_session_info['bytes_processed'] / $progress_session_info['content_length']) * 100;
