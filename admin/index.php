...
foreach ($adm_extensions as $m) {
    ...
    $adm_extensions_menu[$i] = [
        ...
        'lang' => !empty($lang['R_' . strtoupper($m)])
            ? $lang['R_' . strtoupper($m)]
            : (!empty($olang['R_' . strtoupper($m)])
                ? $olang['R_' . strtoupper($m)]
                : strtoupper($m)),
        'link' =>
            './' .
            basename(ADMIN_PATH) .
...
if (isset($go_menu)) {
    foreach ($go_menu as $m => $d) {
        $go_menu_html .=
            ...
            $d['link'] .
            '" ' .
            (!empty($d['confirm']) ? ' onclick="javascript:return confirm_form();"' : '') .
            '>' .
            $d['name'] .
            '</a></li>';
    }
...
if (!ig('_ajax_')) {
    ...
    echo_ajax(1, empty($adminAjaxContent) ? $tpl->display($stylee, $styleePath) : $adminAjaxContent, $go_menu_html);
}
